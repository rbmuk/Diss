...
\begin{align*}
    x^* \in \mathrm{argmin}_{x \in \R^d} f(x)
\end{align*}
...
\begin{align*}
    \cph_\sigma(z) = \frac{1}{(2\pi \sigma^2)^{d/2}} \exp(-\frac{\mg{z}^2}{2})
\end{align*}
...
\begin{align*}
    |\E[\widetilde{\grad} f(x)] - \grad f(x)| = O_d(L \sigma \sqrt{d})
\end{align*}
...
In an ML model $\Phi_\theta(x)$, the parameters $\theta \in \R^{d \times m}$ are naturally thought of not as vectors but as matrices. While helpful that the estimator~\ref{eq:estimator} lets us evaluate at only $r$ perturbations per iteration rather than the full $d \times m$, since $p \in \R^{d \times m}$ has all Gaussian entries, every entry is non-zero almost surely. Thus evaluating $\Phi_{\theta + p}(x)$ is still slow as we need to first add the two matrices $\theta + p$ before evaluating, with memory $O(rd)$ just to store the perturbations.

The low rank trick, introduced in~\cite{sarkar2025evolution}, is to sample $a \sim \mathcal N(0,I_d)$, $b \sim \mathcal N(0, I_m)$, and exploit $(\theta + ab^\top)x = \theta x + a(b^\top x)$. This reduces the memory to $O(r(d+m))$, which makes zero-order algorithms practical.
...
\begin{definition}
    The random variable $X \in \R^d$ is isotropic if
    \begin{align*}
        \E[XX^\top] = I
    \end{align*}
...
\begin{definition}
    For a state space $\Omega$, $\mathcal F \subset 2^X$ is a $\sigma$-algebra if:
...
\begin{definition}
    ...
    \begin{align*}
        \P(\bigcup_{i=0}^{\infty} E_i) = \sum_{i=0}^{\infty} E_i
    \end{align*}
\end{definition}
