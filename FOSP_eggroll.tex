...
\chapter[EGGROLL for Nonconvex Optimization]
{EGGROLL for Nonconvex Optimization}
\label{chap:fosp-eggroll}
In zeroth-order methods, how do GPU clusters actually share perturbations with each other? \textit{A priori}, they would have to send the full weights of each perturbation to every cluster, which could take prohibitively long due to synchronization requirements. This would effectively eliminate the gains behind evaluating $r$ perturbations in parallel we have discussed in detail above.

\citet{salimans2017evolution} solved this problem by sharing only the pseudorandom seeds used to generate each Gaussian perturbation. Full high-dimensional perturbations can then be shared between GPUs using only a 32-bit seed, where the receiving GPU reconstructs the perturbation locally. For the first time, zeroth-order methods, also called Evolutionary Strategies (ES), were scaled to machine learning models with millions of parameters, using more than one thousand parallel workers. But as we have seen in Chapter~\ref{chap:linear-mixing}, increasing the population size $r$ linearly speeds up the algorithm. A twofold increase in the number of parallel workers yields precisely a twofold reduction in training cost. 

The bottleneck now, having solved the problem of sharing the perturbations across GPUs, was memory. Each Gaussian perturbation has $dm$ nonzero entries almost surely, where $dm$ is the dimension of the model. Thus, for population size $r$, each GPU needs to store $O(dm \cdot r)$ parameters. This fundamentally limits how big $r$ can be in practice. 

Very recently, \citet{sarkar2025evolution} introduced EGGROLL to solve this bottleneck by replacing full Gaussian perturbations $P \sim \mathcal N(0, I_{d \times m})$ with rank-$k$ perturbations $A \in \R^{d \times k}$ and $B \in \R^{m \times k}$ with $E = \frac{1}{\sqrt{k}} AB^\top$, critically using $u(M+\sigma E)^\top = uM^\top + \frac{\sigma}{\sqrt{k}} (uB) A^\top$. Using this low-rank trick, they were able to scale ES vastly further than done previously. On the theoretical side, they showed that, under some reasonable assumptions if $\sigma = o(d^{-1/2})$ then as $d \to \infty$,
\begin{align*}
    \frac{1}{\sigma}\mbb E_{P \sim \mathcal N(0,I_{d\times m})}[f(X+\sigma P)P] \to \grad f(X).
\end{align*}
Furthermore, they prove that as the rank of the perturbations $k\to\infty$, 
\begin{align*}
    \frac{1}{\sigma} \mbb E_{E \sim p(E)}\left[E f\left(X + \sigma E\right)\right] \to \frac{1}{\sigma}\mbb E_{P \sim \mathcal N(0,I_{d \times m})}[f(X+\sigma P)P].
\end{align*}
This is because the distribution of $E$ will approach $\mathcal N(0, I_{d \times m})$ with increasing rank. 

\section{Main Result}
The preceding result suggests that increasing the rank of the perturbations is beneficial, so the resulting distribution becomes Gaussian. The Gaussian distribution is rotationally invariant and the rank-1 update $ab^\top$ is not, maybe we need this property to ``locate'' the gradient. Rejecting this, we prove that despite only using rank-1 perturbations $ab^\top$, not only does the algorithm converge to a first-order stationary point, but that its speed also scales linearly with the population size. The practical takeaway is that when function evaluations are performed in parallel, twice as much compute converges twice as fast. 
...
In this chapter we work with symmetric EGGROLL, following the low-rank perturbation framework of \citet{sarkar2025evolution}.
\begin{definition}[Symmetric EGGROLL Update]
    \label{def:symm-eggroll-update}
    We define a population $r$ symmetric zeroth order estimator as follows:
    \begin{align*}
...
\begin{algorithm}[H]
\caption{Population-\(r\) Symmetric EGGROLL}
\label{alg:symmetric-eggroll}
\begin{algorithmic}
    \Require Initial point \(X_0 \in \mathbb R^{d\times m}\), horizon \(T\), stepsize \(\eta>0\), smoothing radius \(\sigma>0\), population size \(r\in \mathbb N\)
    \For{\(t = 0,\ldots,T-1\)}
        \State Sample $\set{a_{t,i}}_{i=1}^r \sim \mathcal N(0,I_d)$ and $\set{b_{t,i}}_{i=1}^r \sim \mathcal N(0,I_m)$ to compute $g_\sigma^r(X_t)$
            
        \State Update $X_{t+1}
            \gets
            X_t-\eta g_\sigma^r(X_t)$
    \EndFor
    \State \Return $X_t$ for $t \sim \mathrm{Unif}([0, T-1])$
\end{algorithmic}
\end{algorithm}
...
The overall proof strategy follows that of~\citet{ren2023escaping}. The main new ingredient is Theorem~\ref{thm:error_concentration}, which establishes the concentration bound required when full Gaussian perturbations are replaced by rank-one perturbations. The Gaussian argument does not extend to this setting because rank-one perturbations have fat tails, with entries that are sub-exponential rather than subGaussian. 

\section{Concentration Bounds}
Here we prove the novel concentration bound about the EGGROLL error. Throughout this chapter, all matrix norms are the Frobenius norm
\begin{align*}
...
We have seen subGaussian random variables in Chapter~\ref{chap:preliminaries}. Here, we will need a generalization, called norm-subGaussian.
\begin{definition}
    The random vector $X$ is norm-subGaussian with parameter $\sigma$, abbreviated $\mathrm{nSG}(\sigma)$, if
    \begin{align*}
        \E[\exp(\frac{\mg{X}^2}{3\sigma^2})] \leq 2
    \end{align*}
\end{definition}
As an example, $p \sim \mathcal N(0,I_d)$ is $\mathrm{nSG}(\sqrt d)$. The next lemma is basically a Hoeffding-like inequality for norm-subGaussian random variables. 
\begin{lemma}[\cite{jin2019short}]\label{lem:nsg-hoeffding}
    Let random vectors $X_1, \ldots, X_T \in \R^d$, and corresponding filtrations $\mathcal F_t = \sigma(X_1, \ldots, X_t)$ satisfy that 
    \begin{align*}
        \Et[X_t] = 0, \quad \Pt(\mg{X_t} \geq s) \leq 2e^{-\frac{s^2}{3\sigma_t^2}} \quad \forall s \geq 0
    \end{align*}
    Then there exists an absolute constant $c$ such that for any fixed $\delta > 0, \theta > 0$, with probability at least $1-\delta$,
    \begin{align*}
        \left\|\sum_{t=1}^T X_t\right\| \leq c\cdot \theta \sum_{t=1}^T \sigma_t^2 + \frac{1}{\theta} \log(\frac{2d}{\delta}).
    \end{align*}
\end{lemma}

This will help us later.
\begin{lemma}
    \label{lem:bounded-p-norm}
...
\begin{proof}
    ...
    We want
    \begin{align*}
        \E[(X-d) ; A] = \int_L^U (x-d)f_d(x)\,\dd x = 2Lf_d(L) - 2Uf_d(U) \overset{\mathrm{want}}{=} 0.
    \end{align*}
    Therefore, we need 
    \begin{align*}
    ...
    \end{align*}
    When $\sqrt{3e} \delta < 1$, this is $< \sqrt{3e} \delta$, otherwise this bound holds vacuously. Therefore $\P(X \leq L) \leq \sqrt{3e} \delta$. On the other hand, using that for $t < 1/2$, where $Z \sim \mathcal N(0,I)$,
    \begin{align*}
...
Here we present the novel concentration bound. The proof is loosely inspired by Proposition 2 of \citet{ren2023escaping}, but the majority of the argument is different.

\begin{theorem}
    \label{thm:error_concentration}
    ...
    \end{align*}
    Then, there exist some absolute constants $C > 0, c > 0$ such that for any $\tau \in \bZ^+$, and $0 < \delta < 1/e$, for any $\theta > 0$, with probability at least $1-\delta$ given $\mathcal F_{\tau-1}$, 
    \begin{align*}
...
\begin{proof}
    Let $V_t = P\Sigma Q^\top = \sum_{i=1}^s \sigma_i p_i q_i^\top$, where $s=\operatorname{rank}(V_t)$, be the singular value decomposition of $V_t$. Extend $\set{q_1, \ldots, q_s}$ to an orthonormal basis $\set{q_1, \ldots, q_m}$ of $\R^m$. Define the events
    \begin{align*}
    ...
    \end{align*}
    Let $R$ be an orthogonal matrix $RR^\top = R^\top R = I$, and notice that $Ra_t$ has the same distribution as $a_t$, and that $A_t$ is the same event when $\mg{a_t}$ is replaced with $\mg{Ra_t}$. Therefore,
    \begin{align*}
    ...
    \begin{align*}
        \Et[(Q^\top b_t)(Q^\top b_t)^\top ; B_t]_{i,j} &= \Et[(q_i^\top b_t) (q_j^\top b_t) ; B_t] 
    ...
    \end{align*}
    by our judicious choice of $B_{t,i}$ (cf. Lemma~\ref{lem:bounded-p-norm}). 

    As $Q \in \mathcal F_{t-1}$, we thus have
    \begin{align*}
        \Et[Q^\top b_tb_t^\top ; B_t] &= \Et[(Q^\top b_t)(Q^\top b_t)^\top ; B_t] \cdot Q^\top  
        \\&= \Pt(B_t)I \cdot Q^\top  = \Pt(B_t)Q^\top .
    \end{align*}

Therefore,
\begin{align*}
    \Et[V_t - a_ta_t^\top V_t b_tb_t^\top ; A_t\cap B_t] &= V_t\Pt(A_t \cap B_t) - \Et[a_ta_t^\top P \Sigma Q^\top b_tb_t^\top ; A_t\cap B_t]
\end{align*}
Now,
\begin{align*}
\Et[a_ta_t^\top P \Sigma Q^\top b_tb_t^\top ; A_t\cap B_t] &= \Et[a_ta_t^\top ; A_t] P \Sigma \cdot \Et[Q^\top b_tb_t^\top ; B_t] \\
&= P\Sigma Q^\top\Pt(A_t \cap B_t) = V_t\Pt(A_t \cap B_t)
\end{align*}
...
The next part of the proof is showing that $Q_t$ is $\mathrm{nSG}(C\sqrt{md} \mathrm{polylog}(dm/\delta) \mg{V_t}_F)$. First, notice that on $B_t$,
\begin{align*}
    \mg{b_t}^2 = \sum_{i=1}^m |q_i^\top b_t|^2 \leq 3m \log(C/\delta)
\end{align*}
...
\begin{align*}
    \mg{a_t a_t^\top V_t b_t b_t^\top}_F = |a_t^\top V_t b_t| \mg{a_t} \mg{b_t} \leq |a_t^\top V_t b_t| \sqrt{9md \log^2(C/\delta)}
\end{align*}
Our main goal now is to show that $a_t^\top V_t b_t$ is subGaussian with parameter $\widetilde{O}(\mg{V_t}_F)$. To simplify notation, we will drop the $t$s and conditioning, it won't be important for the argument. 
...
\end{align*}
Therefore, as long as $t < 1/(8\sigma_{\max}(V)^2)$,
\begin{align*}
    \E[e^{2t\mg{Vb}^2}] &= \prod_{i=1}^s (1-4t \sigma_i^2)^{-1/2} = \exp\left(\sum_{i=1}^s -\frac 12 \log(1-4t\sigma_i^2)\right) \\
    &\leq \exp\left(4t \sum_{i=1}^s \sigma_i^2\right) = \exp(4t\mg{V}_F^2)
\end{align*}
We are now almost done. Observe, on the event $B$,
\begin{align*}
    \mg{Vb}^2 = b^\top V^\top V b = \sum_{i=1}^s \sigma_i^2 (b^\top q_i)^2 \leq 3\log(C/\delta) \sum_{i=1}^s \sigma_i^2 = 3\log(C/\delta) \mg{V}_F^2
\end{align*}
Now, $t\mg{Vb}^2 \leq t 3\mg{V}_F^2 \log(C/\delta)$, so choosing $t = 1/(24\mg{V}_F^2 \log(C/\delta))$, we will satisfy all the bounds above ($\sigma_{\max}(V)^2 \leq \mg{V}_F^2$), and get
\begin{align*}
    \E[e^{t(a^\top V b \1_{A \cap B})^2}] = \E[\E[e^{t (a^\top Vb \1_{A \cap B})^2} \mid b]] \leq e^{4t \mg{V}_F^2} \leq \exp(\frac 1{6\log(C/\delta)}) \leq e^{1/6} \leq 2
\end{align*}
Therefore, $a_t^\top V_t b_t$ is $\mathrm{nSG}(\sqrt{8 \log(C/\delta)} \mg{V_t}_F)$. Combining the previous bounds on $\mg{a}$ and $\mg{b}$, we get
\begin{align*}
    \Et[\exp(\frac{\mg{a_ta_t^\top V_t b_tb_t^\top \1_{A_t \cap B_t}}_F^2}{c' md \log^3(C/\delta)\mg{V_t}_F^2})] \leq 2
\end{align*}
Therefore, using $\mg{Q_t}_F^2 \leq 2\mg{V_t}_F^2 + 2\mg{a_ta_t^\top V_t b_tb_t^\top \1_{A_t \cap B_t}}_F^2$, for some absolute constant $c'$,
\begin{align*}
    \Et[\exp(\frac{\mg{Q_t}_F^2}{c' md \log^3(C/\delta)\mg{V_t}_F^2})] \leq 2
\end{align*}
This proves that $Q_t$ is $\mathrm{nSG}(c'\sqrt{md}\log^{3/2}(C/\delta) \mg{V_t}_F)$. 

Thus, by Lemma~\ref{lem:nsg-hoeffding}, for some absolute constant $c$, with probability at least $1-\delta$,
\begin{align*}
    \left \|\sum_{t=0}^{\tau-1} Q_t \right\|_F \leq c \cdot \theta \sum_{t=0}^{\tau-1} md \log^{3}(C/\delta) \mg{V_t}_F^2 + \frac{1}{\theta} \log \frac{2md}{\delta}.
\end{align*}
...
\begin{align*}
    \mbb P_{\tau-1} \left(\bigcap_{t=0}^{\tau-1} A_t \cap B_t \right) \geq 1 - \tau(m+1)\delta
\end{align*}
so, with probability at least $1-\delta - \tau(m+1)\delta$, 
\begin{align*}
    \left \| E_\tau \right \|_F \leq c \cdot \theta \sum_{t=0}^{\tau-1} md \log^{3}(C/\delta) \mg{V_t}_F^2 + \frac{1}{\theta} \log(\frac{2md}{\delta}).
\end{align*}
After rescaling $\delta$ to $\delta / (1 + \tau (m+1))$, we have arrived at the desired result.
\end{proof}


We will need this definition to bound the quadratic error. 
\begin{definition}
...
\begin{lemma}\label{lem:noise-norm}
    Suppose $a_0, \ldots, a_{\tau-1} \sim \mathcal N(0, I_d)$, and $b_0, \ldots, b_{\tau-1} \sim \mathcal N(0, I_m)$. Then for any $k > 0$ there exist absolute constants $c,C > 0$ such that for any $0 < \delta < 1$, w.p. $ \geq 1 -\delta$,
    \begin{align*}
        \sum_{t=0}^{\tau-1} \mg{a_tb_t^\top}^{2k}_F \leq \tau Cc^{2k} d^k m^k (1+\log(1/\delta)^{2k}) 
    \end{align*}
    When $\delta < 1/e$, 
    \begin{align*}
        \sum_{t=0}^{\tau-1} \mg{a_tb_t^\top}^{2k}_F \leq 2\tau Cc^{2k} d^k m^k \log(1/\delta)^{2k}.
    \end{align*}
\end{lemma}

\begin{proof}
    $(a_t)_j^2$ and $(b_t)_\ell^2$ are $SW(1,1)$. Thus their product is $SW(c, 2)$. Thus $\mg{a_tb_t^T}_F^2 = \sum_{j,\ell} (a_t)_j^2 (b_t)_\ell^2$ is $SW(cmd, 2)$ for some absolute constant $c$. Thus, $\mg{a_tb_t^\top}_F^{2k}$ is $SW(c^km^kd^k, 2k)$. Therefore by the lemma, for $s \geq \tau Cc^km^kd^k$,
    \begin{align*}
        \P(\sum_{t=0}^{\tau-1} \mg{a_tb_t^\top}_F^{2k} \geq s) \leq \exp(-\qty(\frac{s}{\tau Cc^kd^km^k})^{1/(2k)})
    \end{align*}
...
\begin{lemma}[Function decrease for population $r$ symmetric EGGROLL]\label{lem:symm-eggroll-function-decrease}
    Suppose at each time $t$, the algorithm performs the update step with population $r \leq dm$,
    \begin{align*}
        X_{t+1} = X_t - \eta g_\sigma^{r}(X_t)
    \end{align*}
    where
    \begin{align*}
        g_\sigma^{r}(X_t) \coloneqq \frac{1}{r} \sum_{i=1}^r \frac{f(X_t + \sigma P_{t,i}) - f(X_t - \sigma P_{t,i})}{2\sigma} P_{t,i}
    \end{align*}
    where $P_{t,i} = a_{t,i}b_{t,i}^\top$ and each $a_{t,i},b_{t,i}$ are drawn i.i.d. from $\mathcal N(0, I_d)$, and $\mathcal N(0, I_m)$ respectively, and $\sigma > 0$ is the smoothing radius.

    Then there exist absolute constants $c>0,C \geq 1$ such that, for any $T \in \bZ^+$ and $1 \leq \tau' \leq T$, and $0 < \delta < 1/e$, then upon defining $\mathcal H_{\tau}(\delta)$ to be the event on which the inequality 
    \begin{align*}
...
\begin{align*}
    \mathcal F_{-1} = \sigma(X_0), \quad \mathcal F_t = \sigma \left(X_0, \set{a_{0,i}}_{i=1}^r, \ldots, \set{a_{t,i}}_{i=1}^r, \set{b_{0,i}}_{i=1}^r, \ldots, \set{b_{t,i}}_{i=1}^r \right)        
\end{align*}
...
\begin{align*}
    g_\sigma^r(X_t) = \frac 1r \sum_{i=1}^r \frac{f(X_t+\sigma P_{t,i}) - f(X_t - \sigma P_{t,i})}{2 \sigma} P_{t,i} = \frac{1}{r} \sum_{i=1}^r \langle\grad f(X_t+\sigma s_{t,i} P_{t,i}),P_{t,i}\rangle_F P_{t,i}.
\end{align*}
Then we have that
\begin{align*}
...
\begin{align*}
    &f(X_{t+1}) - f(X_t) 
    \\&\leq  \langle X_{t+1} - X_t, \grad f(X_t) \rangle + \frac L2 \mg{X_{t+1} - X_t}^2 \\
    &\leq - \eta \cdot \frac{1}{r} \sum_{i=1}^r |\langle \grad f(X_t), P_{t,i} \rangle|^2 
    \\& \quad - \eta \cdot \frac{1}{r} \sum_{i=1}^r \langle \grad f(X_t), P_{t,i} \rangle \langle \grad f(X_t + \sigma s_{t,i} P_{t,i}) - \grad f(X_t), P_{t,i} \rangle \\
...
\begin{align*}
    &- \frac{\eta}{r} \sum_{i=1}^r \langle \grad f(X_t), P_{t,i} \rangle \langle \grad f(X_t + \sigma s_{t,i} P_{t,i}) - \grad f(X_t), P_{t,i} \rangle
    \\&\leq \frac{\eta}{2r} \sum_{i=1}^r \left(|\langle \grad f(X_t ), P_{t,i} \rangle|^2 + \sigma^2 L^2 \mg{P_{t,i}}^4 \right)
\end{align*}
Also, by $\mg{a+b}^2 \leq 2\mg{a}^2 + 2\mg{b}^2$,
\begin{align*}
    &\left \|\frac 1r \sum_{i=1}^r \left(\langle \grad f(X_t), P_{t,i} \rangle P_{t,i} + \langle \grad f(X_t+\sigma s_{t,i} P_{t,i}) - \grad f(X_t), P_{t,i} \rangle P_{t,i} \right) \right \|^2
...
\end{align*}
By Theorem~\ref{thm:error_concentration}, with probability $1-\delta$,
\begin{align*}
    \left \|\sum_{i=1}^r  (\grad f(X_t) - \langle \grad f(X_t), P_{t,i} \rangle P_{t,i}) \right\| \leq c \theta dmr \log^3 \qty(\frac{C r m}{\delta}) \mg{\grad f(X_t)}^2 + \frac{1}{\theta} \log(\frac{Cm^2 dr}{\delta})
\end{align*}
...
Now by Lemma~\ref{lem:noise-norm}, for some absolute constants $C,c$ possibly different from before, with probability at least $1-\delta$,
\begin{align*}
...
\end{align*}
Similarly, we see that, as when $\tau$ is fixed we need only Equation~\ref{eq:error_equation} to hold jointly over $0 \leq t \leq \tau-1$, while the bound on the perturbation norms hold with probability at least $1-2\delta$, so,
\begin{align*}
...
\section{Gradients Don't Move too Quickly}
...
\begin{lemma}\label{lem:block-progress}
    There exists an absolute constant $c_2 \geq 1$ such that, if $\delta > 0$ and
    \begin{align*}
        t_f(\delta) \geq c_2 \frac{1}{r}\log(\frac{T}{\delta}) \log^2\left(\frac{Ct_f(\delta)rm T}{\delta} \right), \quad t_f(\delta) \in \bZ,
    \end{align*}
    and 
    \begin{align*}
        \mathcal B_{t_0}(\delta ; k) = \bigcup_{t=t_0}^{t_0+k-1} \set{\frac 1r \sum_{i=1}^r |a_{t,i}^\top \grad f(X_t) b_{t,i}|^2 \geq \frac 12 \mg{\grad f(X_t)}^2}
    \end{align*}
    we have
    \begin{align*}
        \P(\mathcal B_{t_0}(\delta; k)) \geq 1 - \frac{\delta}{T}
    \end{align*}
    for $0 < \delta < 1$, $t_0 \geq 0$ and $k \geq t_f(\delta)$. Note that $t_f(\delta) = \widetilde{O}(1)$. 
\end{lemma}
\begin{proof}
    By using the same technique from the proof of Theorem~\ref{thm:error_concentration}, taking the SVD of $\grad f(X_t) = P \Sigma Q^\top = \sum_{i=1}^s \sigma_i p_i q_i^\top$, upon defining
    \begin{align*}
        B_{i,j}^t = \set{L_\delta < |b_{t,i}^\top q_j| < \sqrt{3\log(CkrmT/\delta)}}, \quad B_{t,i} = \bigcap_{j=1}^m B_{i,j}^t
    \end{align*}
    we know that the set of random variables $\set{\mg{\grad f(X_t)}^2 - |a_{t,i}^\top\grad f(X_t) b_{t,i} \1_{B_{t,i}}|^2}_{i=1}^r$ are independent, mean zero, $c\mg{\grad f(X_t)}_F^2 \log(CkrmT/\delta)$-subexponential on $B_{t,i}$ given $\mathcal F_{t-1}$. 
    Now define
    \begin{align*}
        E_t = \set{\frac 1r \sum_{i=1}^r |a_{t,i}^\top \grad f(X_t) b_{t,i}|^2 < \frac 12 \mg{\grad f(X_t)}^2}
    \end{align*}
    Therefore, on $\bigcap_{i=1}^r B_{t,i}$,
    \begin{align*}
        \Pt(E_t) &= \Pt(\frac{1}{r} \sum_{i=1}^r |a_{t,i}^\top \grad f(X_t) b_{t,i}|^2 < \frac{1}{2}\mg{\grad f(X_t)}^2) 
        \\&= \Pt(\sum_{i=1}^r \qty(\mg{\grad f(X_t)}^2 - |a_{t,i}^\top \grad f(X_t) b_{t,i}|^2) > \frac r2 \mg{\grad f(X_t)}^2) \\&\leq \exp(-c' \min\set{\frac{r}{4\log^2(CkrmT/\delta)}, \frac{r}{2\log(CkrmT/\delta)}}) 
        \\&\leq \exp(-c'' \frac{r}{\log^2(CkrmT/\delta)})
    \end{align*}
    for some absolute constant $c''$, where the last line follows by using Bernstein concentration inequality.

    Now,
    \begin{align*}
        \P(\bigcap_{t=t_0}^{t_0+k-1} E_t) &= \P(\bigcap_{t=t_0}^{t_0+k-1} \set{\frac 1r \sum_{i=1}^r |a_{t,i}^\top \grad f(X_t) b_{t,i}|^2 < \frac 12 \mg{\grad f(X_t)}^2})  \\
        &= \E[\prod_{t=t_0}^{t_0+k-2} \1_{E_t} \mathbb E_{\mathcal F_{t_0+k-2}}[\1_{E_{t_0+k-1}}]] \leq \cdots \leq \exp(-c'' rk / \log^2(CkrmT/\delta))
    \end{align*}
    therefore by choosing 
    \begin{align*}
        k \geq c'' \frac{1}{r}\log(T/\delta) \log^2(Ckrm T / \delta),
    \end{align*} 
    we have, on $\bigcap_{t=t_0}^{t_0 + k - 1} \bigcap_{i=1}^r B_{t,i}$,
    \begin{align*}
        \P(\bigcap_{t=t_0}^{t_0+k-1} \set{\frac{1}{r} \sum_{i=1}^r |a_{t,i}^\top \grad f(X_t) b_{t,i}|^2 < \frac 12 \mg{\grad f(X_t)}^2}) \leq \frac{\delta}{2T}.
    \end{align*}
    Furthermore, 
    \begin{align*}
        \P(\bigcap_{t=t_0}^{t_0 + k - 1} \bigcap_{i=1}^r B_{t,i}) \geq 1- \frac{\delta}{2T}
    \end{align*}
    which completes the proof.
\end{proof}

\begin{lemma}\label{lem:gradient-change}
    Define
    \begin{align*}
    ...
    \begin{align}
        \label{eq:gradient-change-step-size}
...
\begin{proof}
    ...
    \end{align*}
    Now for some absolute constant $c_2 > 0$ by Theorem~\ref{thm:error_concentration}, with probability at least $1-\delta/T$,
    \begin{align*}
...
\section{Progress Lower Bound}

This next very important lemma is the key that will let us lower bound 
\begin{align*}
    \sum_{t=0}^{\tau-1} \frac 1r \sum_{i=1}^r |\langle \grad f(X_t), P_{t,i} \rangle|^2 \geq \tilde \Omega \qty(\sum_{t=0}^{\tau-1} \mg{\grad f(X_t)}^2)
\end{align*}
in some sense.
...
\begin{lemma}\label{lem:half-two-gradient}
    Let $0 < \delta \leq 1/e$ and $T \in \bZ^+$ satisfying $T \geq 2t_f(\delta) + 1$. Consider any positive integer $t_f' \leq 2t_f(\delta)$, and any $t_0 \in \set{0, \ldots, T-1-t_f'}$. Suppose $\eta$ satisfies~\eqref{eq:gradient-change-step-size}, then on
    \begin{align*}
        \mathcal E(t_0, t_f', \delta) \cap \qty(\bigcap_{t=t_0}^{t_0+t_f'-1} \mathcal A_t(\delta))
    \end{align*}
    we have
    \begin{align*}
        \frac 12 \mg{\grad f(X_{t_0})} \leq \mg{\grad f(X_t)} \leq 2 \mg{\grad f(X_{t_0})}
    \end{align*} 
    for all $t \in \set{t_0, \ldots, t_0+t_f'-1}$. 
\end{lemma}
\begin{proof}
    By plugging in the definition for $\mathcal E(t_0,t_f',\delta)$ into the definition of $\mathcal A_t(\delta)$, we see that on the prescribed event,
    \begin{align*}
    ...
    which leads to
    \begin{align*}
        \qty(1 - \frac{1}{4t_f(\delta)})^{t-t_0} \mg{\grad f(X_{t_0})} \leq \mg{\grad f(X_t)} \leq \qty(1+\frac{1}{4t_f(\delta)})^{t-t_0} \mg{\grad f(X_{t_0})}.
    \end{align*}
...
\begin{lemma}\label{lem:bounded-perturbation-cubes}
...
\end{lemma}
\begin{proof}
    This is immediate from Lemma~\ref{lem:noise-norm} and the Cauchy-Schwarz inequality.
\end{proof}

\begin{lemma}\label{lem:gradient-dominates}
    Let $0 < \delta \leq 1/e$ and $T \in \bZ^+$ such that $T > 2t_f(\delta) + 1$. Consider any positive integer $t_f' \leq 2t_f(\delta)$ and $t_0 \in \set{0, \ldots, T-1-t_f'}$. Suppose $\eta$ satisfies~\eqref{eq:gradient-change-step-size}. Then on the event 
    \begin{align*}
...
\begin{proof}
    ...
    \end{align*}
    For $t_0\leq j<t'$, using the idea from Lemma~\ref{lem:half-two-gradient},
    \begin{align*}
        \mg{\grad f(X_j)} \leq 2 \mg{\grad f(X_{t'})} \leq 16 t_f(\delta)\eta L^2 \sigma c(dm)^{3/2} \log(T/\delta)^3
    \end{align*}
    On the other hand, for $t \in \set{t', \ldots, t_0+t_f'-2}$, by using definition of $\mathcal A_t(\delta)$ and $\mathcal G_t(\delta)$, we get
    \begin{align*}
        \mg{\grad f(X_{t+1})} &\leq \qty(1 + \frac{1}{8t_f(\delta)})\mg{\grad f(X_t)} + \eta \sigma L^2 c(dm)^{3/2} \log(T/\delta)^3
        \\&\leq \qty(1+\frac{1}{8t_f(\delta)})^{t+1-t'} \mg{\grad f(X_{t'})} 
        \\&\quad + \sum_{i=0}^{t-t'} \qty(1+\frac{1}{8t_f(\delta)})^{t-t'-i} \eta \sigma L^2 c(dm)^{3/2} \log(T/\delta)^3 \\
        &\leq \qty(1+\frac{1}{8t_f(\delta)})^{t_f'} \mg{\grad f(X_{t'})}
...
\begin{lemma}[Function change for large $\tau$]\label{lem:function-change-for-large-tau}
    ...
    \begin{align*}
        I_1 \coloneqq \set{0 \leq k \leq K-1 \,:\, \mg{\grad f(X_t)} \geq 8t_f(\delta) \eta \sigma L^2 \frac 1r \sum_{i=1}^r \mg{P_{t,i}}^3  \text{ for all $t \in J_k$}}
    \end{align*}
    ...
    Then on the event
    \begin{align*}
        \mathcal E_\tau(\delta) \coloneqq \mathcal H_\tau(\delta) \cap \qty(\bigcap_{t=0}^{\tau-1} \mathcal A_t(\delta)) \cap \qty(\bigcap_{t=0}^{\tau-1} \mathcal G_t(\delta) ) \cap \qty(\bigcap_{k=0}^{K-1} \mathcal B_{kt_f(\delta)}(\delta ; t_f(\delta)))
    \end{align*}
...
\begin{proof}
    Any interval $J_k = \set{t_0, \ldots, t_0+t_f(\delta)-1}$ belongs to one of the following two cases:
    ...
    \end{align*}
    By the Lemma~\ref{lem:half-two-gradient}
    we have
    \begin{align*}
    ...
    \begin{align*}
        \frac 14 \sum_{t \in J_k} \frac 1r \sum_{i=1}^r |\langle P_{t,i}, \grad f(X_t) \rangle|^2 &\geq \frac 18 \min_{t \in J_k} \mg{\grad f(X_t)}^2 \geq \frac{1}{128} \max_{t \in J_k} \mg{\grad f(X_t)}^2
        \\& \geq \frac{1}{128t_f(\delta)} \sum_{t\in J_k} \mg{\grad f(X_t)}^2
    \end{align*}
    By choosing 
    \begin{align*}
        \frac{\eta cdm \chi^4}{r} \leq \frac{1}{128 t_f(\delta)}
    \end{align*}
    we have
    \begin{align*}
    ...
    \end{align*}
    Then by Lemma~\ref{lem:gradient-dominates}, we know that, for all $t \in J_k$,
    \begin{align*}
        \mg{\grad f(X_t)} \leq 16 t_f(\delta) \eta \sigma L^2 c(dm)^{3/2} \log(T/\delta)^3
    \end{align*}
    ...
    \end{align*}
    and thus by the definition of $\mathcal H_\tau(\delta)$ in Lemma~\ref{lem:symm-eggroll-function-decrease},
    \begin{align*}
...
\begin{theorem}[Symmetric EGGROLL Convergence]\label{thm:symm-eggroll-conv}
    Let $0 < \delta < 1/e$ be arbitrary. Define $\chi\coloneqq\log(Cm^2drT/\delta)$. Suppose we choose $\sigma, \eta$ and $T$ such that
    \begin{align*}
        \sigma^2 &\leq \min \set{\frac{\ve^2}{1024 C c^4 L^2 (dm)^2 \log(T/\delta)^4}, \frac{\ve^2}{8096 rL^3 (dm)^2 \log(T/\delta)^2}} \\
        \eta &\leq \frac{1}{8t_f(\delta)} \min\set{\sqrt{\frac{r}{dm}} \cdot \frac{1}{c L \log(CdmrT/\delta)^2}, \frac{r}{dm} \cdot \frac{1}{8c\chi^4}}, \\
...
\begin{proof}
    ...
    \end{align*}
    For the rest of the proof assume we are working on $\mathcal E_T(\delta)$. By Lemma~\ref{lem:function-change-for-large-tau}, and our choices of $\eta$ and $\delta$ in the statement of the theorem, we have
    \begin{align*}
    ...
    \end{align*}
    While by Lemma~\ref{lem:gradient-dominates}, if $k \in I_1^c$ and $t \in J_k$,
    \begin{align*}
        \mg{\grad f(X_t)} \leq 16 t_f(\delta) \eta \sigma L^2 c(dm)^{3/2} \log(T/\delta)^3
    \end{align*}
    Therefore, $I_\ve \subset I_1$. Now by Lemma~\ref{lem:half-two-gradient}, for any $k \in I_1$, 
    \begin{align*}
    ...
    \begin{align*}
        -\sum_{k \in I_1} \frac{\eta}{4} \min_{t \in J_k} \mg{\grad f(X_t)}^2 \leq -\sum_{k \in I_\ve} \frac{\eta}{4} \cdot \frac{\ve^2}{16} \leq -\frac{T}{4t_f(\delta)} \cdot \frac{\eta}{4} \cdot \frac{\ve^2}{16} = -\frac{T \eta \ve^2}{256 t_f(\delta)}
    \end{align*}
    Hence,
    \begin{align*}
    ...
    Finally,
    \begin{align*}
        \P(\mathcal E_T^c) &\leq \P(\mathcal H_T^c) + \sum_{t=0}^{T-1} \P(\mathcal A_t^c(\delta)) + \sum_{t=0}^{T-1} \P(\mathcal G_t(\delta)^c) + \sum_{k=0}^{K-1} \P(\mathcal B_{kt_f(\delta)}(\delta ; t_f(\delta))^c) \\
