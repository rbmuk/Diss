...
\citet{salimans2017evolution} solved this problem by sharing only the pseudorandom seeds used to generate each Gaussian perturbation. Full high-dimensional perturbations can thus be shared between GPUs using only a 32-bit seed, where the receiving GPU subsequently reconstructs the perturbation locally. For the first time, zeroth-order methods, also called Evolutionary Stratregies (ES), was scaled to machine learning models with millions of parameters, using more than one thousand parallel workers. But as we have seen in Chapter~\ref{chap:linear-mixing}, increasing the population size $r$ linearly speeds up the algorithm. A two-times increase in the number of parallel workers yields precisely a two-times speedup in training speed. 
...
The overall proof strategy follows that of~\citet{ren2023escaping}. The main new ingredient is Theorem~\ref{thm:error_concentration}, which establishes the concentration bound required when full Gaussian perturbations are replaced by rank-one perturbations. The Gaussian argument does not extend to this setting because rank-one perturbations have fat tails, with entries that are sub-exponential rather than sub-gaussian. 

\section{Concentration Bounds}
Here we prove the novel concentration bound about the EGGROLL error. Throughout this excerpt, all norms of matrices is the Frobenius norm
\begin{align*}
...
\end{align*}
Therefore, $a_t^\top V_t b_t$ is $\mathrm{nSG}(\sqrt{24 \log(C/\delta)} \mg{V_t}_F)$. Combining the previous bounds on $\mg{a}$ and $\mg{b}$, we get
\begin{align*}
    \Et[\exp(\frac{\mg{a_ta_t^\top V_t b_tb_t^\top \1_{A_t \cap B_t}}_F^2}{216 md \log^3(C/\delta)\mg{V_t}_F^2})] \leq 2
\end{align*}
Therefore, using $\mg{Q_t}_F^2 \leq \mg{V_t}_F^2 + \mg{a_ta_t^\top V_t b_tb_t^\top \1_{A_t \cap B_t}}_F$, for some absolute constant $c'$,
\begin{align*}
...
\begin{lemma}\label{lem:block-progress}
    ...
    \begin{align*}
        t_f(\delta) \geq c'' \frac{1}{r}\log(\frac{T}{\delta}) \log^2\left(\frac{Ct_f(\delta)rm T}{\delta} \right), \quad \delta > 0,
    \end{align*}
    and 
    \begin{align*}
...
\end{lemma}
\begin{proof}
    By using the same technique from the proof of Theorem~\ref{thm:error_concentration}, taking the SVD of $V = P \Sigma Q^\top = \sum_{i=1}^s \sigma_i p_i q_i^\top$, upon defining
    \begin{align*}
...
\end{lemma}
\begin{proof}
    Obvious from Lemma~\ref{lem:noise-norm} and the Cauchy-Schwarz inequality.
\end{proof}

\begin{lemma}\label{lem:gradient-dominates}
    Let $0 < \delta \leq 1/e$ and $T \in \bZ^+$ such that $T > 2t_f(\delta) + 1$. Consider any positive integer $t_f' \leq 2t_f(\delta)$ and $t_0 \in \set{0, \ldots, T-1-t_f'}$. Suppose $\eta$ satifies~\eqref{eq:gradient-change-step-size}. Then on the event 
    \begin{align*}
...
\begin{proof}
    ...
    \begin{align*}
        \frac 14 \sum_{t \in J_k} \frac 1r \sum_{i=1}^r |\langle P_{t,i}, \grad f(X_t) \rangle|^2 &\geq \frac 14 \min_{t \in J_k} \mg{\grad f(X_t)}^2 \geq \frac{1}{64} \max_{t \in J_k} \mg{\grad f(X_t)}^2
...
\begin{proof}
    ...
    \end{align*}
    While by Lemma~\ref{lem:gradient-dominates}, if $k \in I_1^c$, 
    \begin{align*}
        \mg{\grad f(X_t)} \leq 16 t_f(\delta) \eta \sigma L^2 c(dm)^{3/2} \log(T/\delta)^3
    \end{align*}
