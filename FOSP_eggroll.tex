...
In previous chapters we have largely focused on the theoretical aspects of zero-order optimization methods. However, their implementation and practical use is also an important question. While we have focused on zero-order methods, how do large GPU clusters actually share the perturbations with each other? \textit{A priori}, they would have to send the full weights of each perturbation to every cluster, which could take prohibitively long due to synchronization requirements. This would effectively eliminate the gains behind evaluating $r$ perturbations in parallel we have discussed in detail above.

~\cite{salimans2017evolution} solved this problem by sharing only the pseudorandom seeds used to generate each Gaussian perturbation. Full high-dimensional perturbations can thus be shared between GPUs using only a 32-bit seed, where the receiving GPU subsequently reconstructs the perturbation locally. For the first time, zeroth-order methods, also called Evolutionary Stratregies (ES), was scaled to machine learning models with millions of parameters, using more than one thousand parallel workers. But as we have seen in Chapter~\ref{chap:linear-mixing}, increasing the population size $r$ linearly speeds up the algorithm. A two-times increase in the number of parallel workers yields precisely a two-times speedup in training speed. 

The bottleneck now, having solved the problem of sharing the perturbations across GPUs, was memory. Each Gaussian perturbation has $d$ nonzero entries almost surely, where $d$ is the dimension of the model. Thus, for population size $r$, each GPU needs to store $O(d \cdot r)$ parameters. This fundamentally limits how big $r$ can be in practice. 

Very recently,~\cite{sarkar2025evolution} introduced EGGROLL to solve this bottleneck by replacing full Gaussian perturbations $P \sim \mathcal N(0, I_{d \times m})$ with rank-$r$ perturbations $A \in \R^{d \times k}$ and $B \in \R^{m \times k}$ with $E = \frac{1}{\sqrt{k}} AB^\top$, critically using $u(M+\sigma P)^\top = uM^\top + \frac{\sigma}{\sqrt{k}} (uB) A^\top$. Using this low-rank trick, they were able to scale ES vastly further than done previously. On the theoretical side, they showed that, under some reasonable assumptions if $\sigma = o(d^{-1/2})$ then as $d \to \infty$,
\begin{align*}
    \frac{1}{\sigma}\mbb E_{p \sim \mathcal N(0,I_d)}[ f(x+\sigma P) P]  \to \grad f(x)
\end{align*}
Furthermore, they prove that as the rank of the perturbations $k\to\infty$, 
\begin{align*}
    \frac{1}{\sigma} \mbb E_{E \sim p(E)}\left[E f\left(M + \sigma E\right)\right] \to \frac{1}{\sigma}\mbb E_{p \sim \mathcal N(0,I_{d \times m})}[ f(x+\sigma P) P]
\end{align*}
this is because the distribution of $E$ will approach $\mathcal N(0, I_{d \times m})$ with increasing rank. 

\section{Main Result}
The preceding result suggests that increasing the rank of the perturbations is beneficial, so the resulting distribution becomes Gaussian. The Gaussian distribution is rotationally invariant and the rank-1 update $ab^\top$ is not, maybe we need this property to ``locate'' the gradient. However, this is not true, it turns out that the requirement for convergence is the isotropic condition mentioned previously in the preliminaries. We prove that, despite only using rank-1 perturbations $ab^\top$, not only do we get convergence to a first-order stationary point, but that its speed also scales linearly with the population size. The practical takeaway is that when function evaluations are performed in parallel, twice as much compute converges twice as fast. 
...
We have seen subGaussian random variables in the preliminaries. Here, we will need a generalization, called norm-subGaussian.
\begin{definition}
    The random variable $X$ is norm-subGaussian with parameter $\sigma$, abbreviated $\mathrm{nSG}(\sigma)$, if
    \begin{align*}
...
\begin{lemma}[\cite{jin2019short}]
    ...
    \begin{align*}
        \Et[x_t] = 0, \quad \Pt(\mg{x_t} \geq s) \leq 2e^{-\frac{s^2}{3\sigma^2}} \quad \forall s \in \R
    \end{align*}
...
\end{align*}
Our main goal now is to show that $a_t^\top V b_t$ is subgaussian with parameter $\widetilde{O}(\mg{V}_F)$. To simplify notation, we will drop the $t$s and conditioning, it won't be important for the argument. 
...
\begin{align*}
    &- \frac{\eta}{r} \sum_{i=1}^r \langle \grad f(X_t), P_{t,i} \rangle \langle \grad f(X_t + \sigma_{t,i} P_{t,i}) - \grad f(X_t), P_{t,i} \rangle 
