...
\begin{itemize}
    ...
    \end{align*}
    iterations, matching the rate for dense Gaussian perturbations while maintaining the property that parallelizing function evaluations across $r$ workers yields a linear-in-$r$ speedup. In particular, Theorem~\ref{thm:error_concentration} and its consequences yield the tools needed to generalize many zeroth-order algorithms to the low-rank setting.
