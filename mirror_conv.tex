...
\label{chap:zo-mirror-descent}
    ...
    Gradient descent iteratively takes a step in the gradient direction, gradually reducing function value. It successively builds better and better upper bounds on $f(x^*)$. However, it ignores the dual problem of finding lower bounds on $f(x^*)$. For convex functions, we have for all $u \in \R^d$,
    \begin{align*}
    ...
    \begin{align}
        ...
        f(u) \geq \frac 1T \sum_{t=0}^{T-1} f(x_t) + \frac{1}{T} \sum_{t=0}^{T-1} \langle \grad f(x_t), u - x_t \rangle
    \end{align}
    ...
    \begin{definition}
        $w: Q \to \R$ is a DGF if it is 1-strongly convex w.r.t. $\mg{\cdot}$, in symbols, $w(y) \geq w(x) + \langle \grad w(x), y-x \rangle + \frac 12 \mg{y-x}^2$. The Bregman divergence is defined
        \begin{align*}
    ...
    Mirror descent's core lemma is this:
    \begin{lemma}[\cite{allenZhu2016linear}]
        \label{lem:mirror-descent}
        If $x_{t+1} = \Mirr_{x_t}(\alpha \grad f(x_t))$, then
        \begin{align*}
            \forall u \in Q,\quad \alpha (f(x_t) - f(u)) \leq \alpha \langle \grad f(x_t), x_t-u \rangle \leq \frac{\alpha^2}{2} \mg{\grad f(x_t)}^2 + V_{x_t}(u) - V_{x_{t+1}}(u).
        \end{align*}
    \end{lemma}
    \begin{proof}
        Notice
        ...
        \end{align*}
        Since $Q$ is convex and $x_{t+1} = \Mirr_{x_t}(\alpha \grad f(x_t))$ is minimal, it satisfies the first-order condition
        \begin{align*}
            \langle \grad V_{x_t}(x_{t+1}) + \alpha \grad f(x_t), u - x_{t+1} \rangle \geq 0 \quad \forall u \in Q
        \end{align*}
        Therefore the above is at most 
        \begin{align*}
            \langle \alpha \grad f(x_t), x_t - x_{t+1} \rangle + \langle - \grad V_{x_t}(x_{t+1}), x_{t+1} - u \rangle
        \end{align*}
    ...
    \begin{lemma}[\cite{allenZhu2016linear}]
        ...
        For $\bar x = \frac{1}{T} \sum_{t=0}^{T-1} x_t$, if we choose $\alpha = \sqrt{2\Theta /(\rho^2 T)}$, given $\Theta \geq V_{x_0}(x^*)$, we have
        \begin{align*}
    ...
    \begin{proof}
        ...
        \begin{align*}
            f(\bar x) - f(x^*) \leq \frac{\alpha}{2} \rho^2 + \frac{1}{T\alpha} \Theta,
        \end{align*}
    ...
    \section{Zeroth-Order Mirror Descent}
    ...
    \begin{definition}[Zeroth-Order Estimator]
        The zeroth-order gradient estimator is defined as follows
        \begin{align*}
    ...
    \begin{theorem}[informal]
        With iterates defined as $x_{t+1} = \Mirr_{x_t}(\alpha \widetilde \grad f(x_t))$, where the perturbation sampled at time $t$ is independent of the past, if $f$ is quadratic and
        \begin{align*}
        ...
        \end{align*}
    \end{theorem}
    This matches the convergence rate of Mirror descent presented in~\cite{allenZhu2016linear}, up to the zeroth-order $d$ penalty and log factors.
    ...
    \begin{align*}
        \alpha (f(x_t) - f(u)) \leq \alpha \langle \grad f(x_t), x_t - u \rangle  = \alpha \langle \widetilde{\grad} f(x_t), x_t - u \rangle + \alpha \langle \grad f(x_t) - \widetilde{\grad} f(x_t), x_t - u \rangle  
    \end{align*}
    ...
    \begin{lemma}
        \label{lem:est-bound}
        With probability at least $1-\delta$, we have for every $0 \leq t \leq T-1$,
        \begin{align*}
            \mg{p_tp_t^\top \grad f(x_t)} \leq c \sqrt{d} \log(cT/\delta) \mg{\grad f(x_t)}
        \end{align*}
    \end{lemma}
    \begin{proof}
        By Lemma~\ref{lem:bounded-p-norm}, the event
        \begin{align*}
            A_t = \set{\mg{p_t} \leq \sqrt{3d\log(C/\delta)}} 
        \end{align*}
        has probability at least $1-\delta$. Now, as $p_t^\top \grad f(x_t) \sim \mathcal N(0, \mg{\grad f(x_t)}^2)$ conditional on $\mathcal F_{t-1}$, we know that
        \begin{align*}
            \E[\exp(\frac{\mg{p_tp_t^\top \grad f(x_t) \1_{A_t}}^2}{s^2})] \leq \E[\exp(\frac{3d \log(C/\delta) |p_t^\top \grad f(x_t)|^2}{s^2})]
        \end{align*}
        Using $\E[e^{\lambda Z^2}] = (1-2\lambda \sigma^2)^{-1/2}$ for $Z \sim \mathcal N(0, \sigma^2)$, we know that
        \begin{align*}
            \Et[\exp(\frac{3d \log(C/\delta) |p_t^\top \grad f(x_t)|^2}{s^2})] = \left(1 - \frac{6d \log(C/\delta)\mg{\grad f(x_t)}^2}{s^2}\right)^{-1/2}
        \end{align*}
        The RHS is $\leq 2$ for $s = \sqrt{8d \log(C/\delta)}\mg{\grad f(x_t)}$. By taking expectations on both sides this bound also holds unconditionally. Then using Markov's inequality,
        \begin{align*}
            &\P(\mg{p_tp_t^\top \grad f(x_t) \1_{A_t}} \geq \sqrt{8d \log(C/\delta)}\mg{\grad f(x_t)} \lambda) 
            \\&= \P(\exp(\frac{\mg{p_tp_t^\top \grad f(x_t) \1_{A_t}^2}}{8d \log(C/\delta) \mg{\grad f(x_t)}^2}) \geq e^{\lambda^2}) \leq 2e^{-\lambda^2}
        \end{align*}
        ...
        \end{align*}
        Since $A_t$ has probability at least $1-\delta$, this bound holds with probability at least $1-2\delta$. Rescaling $\delta \gets \delta/2T$ and changing the absolute constant yields the lemma.
    \end{proof}
    This lemma highlights the main technique for creating concentration bounds when the underlying distribution has fat tails. The first thing you do is truncate with high probability, and prove the resulting random variable has similar scale (whether that be subGaussian, sub-exponential, etc.) as what you want, up to log factors.

    The main idea behind the rest of this proof is that the mean-0 martingale error
    \begin{align*}
        \sum_{t=0}^{T-1} \alpha \langle (I-p_tp_t^\top) \grad f(x_t), x_t-u \rangle
    \end{align*}
    ...
    Let $\set{\mathcal F_t}$ be a filtration. Assume that $g_t,d_t \in \R^d$ are $\mathcal F_{t-1}$-measurable, and assume $p_t \sim \mathcal N(0,I)$ is $\mathcal F_t$-measurable and independent of $\mathcal F_{t-1}$. Define
    \begin{align*}
        E_\tau = \sum_{t=0}^{\tau-1} g_t^\top (I-p_tp_t^\top) d_t
    \end{align*}
    Then for any $\theta > 0$, $0 < \delta < 1/e$, with probability at least $1-\delta$, for some absolute constant $c$,
    \begin{align*}
    ...
    \begin{proof}
        For each $t$ let
        \begin{align*}
            A_t = \set{L_\delta\mg{d_t} \leq |p_t^\top d_t| \leq \sqrt{3 \log(c/\delta)}\mg{d_t}}, \quad \Pt(A_t) \geq 1-\delta
        \end{align*}
        ...
        \begin{align*}
            \E[\exp(\frac{(g^\top p p^\top d \1_A)^2}{t^2})] \leq \E[\exp(\frac{3 \log(C/\delta) \mg{d}^2(p^\top g)^2}{t^2})]
        \end{align*}
        ...
        \end{align*} 
        So, the random variable $Q_t = g_t^\top (I-p_tp_t^\top) d_t \1_{A_t}$ is $\sqrt{24\mg{g_t}^2 \mg{d_t}^2 \log(C/\delta)}$ subGaussian and mean 0 given $\mathcal F_{t-1}$. Thus by the  (subGaussian) Hoeffding inequality with probability at least $1-\delta$ for any $\theta$,
        \begin{align*}
    ...
    \end{proof}
    The preceding lemma is the key for making the argument work. Because $V_x(y) \geq \frac 12 \mg{x-y}^2$, and recalling that $f$ is $\rho$-Lipschitz continuous, we have
    \begin{align*}
...
\begin{proof}
    ...
    \begin{align*}
        \sum_{t=0}^{\tau-1} \alpha (f(x_t) - f(x^*)) &\leq \sum_{t=0}^{\tau-1} \frac{\alpha^2}{2} \mg{\widetilde{\grad} f(x_t)}^2 + V_{x_0}(x^*) - V_{x_\tau}(x^*) 
...
\end{proof}
Naturally, the above lemma should tell us how much $V_{x_t}(x^*)$ can grow. This next lemma tells us an upper bound on how much that is.
\begin{lemma}
...
\end{align*}
is around the same size as $V_{x_0}(x^*)$. This observation is what lets us prove convergence of zeroth-order mirror descent.

\section{Putting it all Together}
\begin{theorem}[Zeroth-Order Mirror Descent Convergence]
    Let $\bar x = \frac{1}{T} \sum_{t=0}^{T-1} x_t$, if we choose
    \begin{align*}
...
The extension of this analysis to the setting without quadratic error is left to future work. The main purpose of this chapter was not merely to show that mirror descent converges, but to determine whether zeroth-order mirror descent can be analyzed with the zeroth-order gradient estimator. Having answered this question affirmatively, the techniques developed here will be applied extensively in the next chapter.
