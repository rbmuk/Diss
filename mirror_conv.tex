...
\label{chap:zo-mirror-descent}
    ...
    \end{definition}
    For simplicity of exposition in this chapter only we will assume there is no quadratic error and $\tilde \grad f(x_t) = p_tp_t^\top \grad f(x_t)$. Thus the above equals,
    \begin{align}
    ...
    \begin{lemma}
        ...
        \begin{align*}
            |p_tp_t^\top \grad f(x_t)| \leq c \sqrt{d} \log(cT/\delta) \mg{\grad f(x_t)}
        \end{align*}
    \end{lemma}
    \begin{proof}
