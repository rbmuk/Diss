...
\label{chap:zo-mirror-descent}
    ...
    \begin{lemma}[\cite{allenZhu2016linear}]
        ...
        For $\bar x = \frac{1}{T} \sum_{t=0}^{T-1} x_t$, if we choose $\alpha = \sqrt{2\Theta /(\rho^2 T)}$, we have
        \begin{align*}
    ...
    \begin{proof}
        ...
        \begin{align*}
            \E[\exp(\frac{\mg{p_tp_t^\top \grad f(x_t) \1_{A_t}}^2}{t^2})] \leq \E[\exp(\frac{3d \log(C/\delta) |p_t^\top \grad f(x_t)|^2}{t^2})]
        \end{align*}
    ...
    Let $\set{\mathcal F_t}$ be a filtration. Assume that $g_t,d_t$ are $\mathcal F_{t-1}$-measurable, and assume $p_t \sim \mathcal N(0,I)$ is $\mathcal F_t$-measurable and independent of $\mathcal F_{t-1}$. Define
    \begin{align*}
    ...
    \begin{proof}
        ...
        \end{align*} 
        So, the random variable $Q_t = g_t^\top (I-p_tp_t^\top) d_t \1_{A_t}$ is $24\mg{g_t}^2 \mg{d_t}^2 \log(C/\delta)$ subgaussian and mean 0 given $\mathcal F_{t-1}$. Thus by the  (subgaussian) Hoeffding inequality with probability at least $1-\delta$ for any $\theta$,
        \begin{align*}
